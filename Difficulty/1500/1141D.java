...
public class Main {
	...
	public static void main(String[] args) {
        ...
        Stack<Integer> i1[] = new Stack[27];
        Stack<Integer> i2[] = new Stack[27]; //indexs of 2nd array;
        
        for(int i=0;i<27;i++)
        {
            i1[i] = new Stack<Integer>();
            i2[i] = new Stack<Integer>();
        }
     ...
     //   System.out.println(Arrays.toString(i2[26].toArray()));
       // System.out.println(Arrays.toString(i1[26].toArray()));
        for(int i=0;i<26;i++)
        {
//System.out.println(Arrays.toString(i2[i].toArray()));
            Stack<Integer> s1 = i1[i]; //I have hs1 indexs;
            Stack<Integer> s2 = i2[i];
            while(s1.size()>0 && s2.size()>0)
            {
                String s = s1.pop() + " " + s2.pop();
                list.add(s);
            }
        }
       // System.out.println(Arrays.toString(list.toArray()));
        //matchin ques of 1 with vals of 2;
        if(i1[26]!=null && i1[26].size()>0)
        {
            for(int i=0;i<26;i++)
            {
                while(i1[26].size()>0 && i2[i].size()>0)
                {
                    String s = i1[26].pop() + " " + i2[i].pop();
                    list.add(s);
                }
            }
        }
        //System.out.println(Arrays.toString(list.toArray()));
        //match ques of 2 with vals of 1;
        if(i2[26]!=null && i2[26].size()>0)
        {
            for(int i=0;i<26;i++)
            {
                while(i2[26].size()>0 && i1[i].size()>0)
                {
                    String s = i1[i].pop() + " " + i2[26].pop();
                    list.add(s);
                }
            }
        }
        ...
        //System.out.println(Arrays.toString(list.toArray()));
        //Matching ques with ques;
        if(i2[26]!=null && i1[26]!=null)
        {
            while(i2[26].size()>0 && i2[26].size()>0)
                {
                    String s = i1[26].pop() + " " + i2[26].pop();
                    list.add(s);
                }
        }
