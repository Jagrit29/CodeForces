...
public class Main {
	public static void main(String[] args) {
        ...
        while(t-- > 0)
        {
            ...
            long need =  (n+1)/2;
            long total = (need)/a * (a+b);  //How many times a is needed; we add (a_b);
            if(need%a==0)
            {
                //means we added b extra;
                total=total-b;
            }
            else
            {
    ...
    public static int solve(char ch[], int i, int dp[])
    {
        if(i>=ch.length-1) return 1;
        if(dp[i]!=-1) return dp[i];
        if(ch[i]==ch[i+1] && (ch[i]=='u' || ch[i]=='n'))
        {
            //taking i and i+1 as m  or n;
            int op1 = solve(ch,i+2,dp);
            
            //taking u as u;
            int op2 = solve(ch,i+1,dp);
            
            return dp[i] = (op1 + op2)%1000000007;
        }
        else
        {
            int op1 = solve(ch,i+1,dp);
            return dp[i] = op1%1000000007;
        }
    }
