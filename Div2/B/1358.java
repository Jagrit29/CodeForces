...
public class Main {
	public static void main(String[] args) {
        ...
        while(t-- > 0)
        {
            ...
            int present=1; //1 is there;
            int call=0;
            Arrays.sort(a);
            for(int i=0;i<n;i++)
            {
                //System.out.println(present+" "+call);
                if(present+call>=a[i])
                {
                    /*
                    if(call==0)
                    ...
                    {
                        present+=call; //call all selected;
                        call=0;
                    }
                    */
                    present=present+call+1;
                    call=0;
                }
                else
                {
                    //Now present is less;
                    //Now this is less, we need to call;
                    call++; //this granny is selected to be called with pair;
                }
            }
