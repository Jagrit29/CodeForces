...
public class Main {
	...
	public static void main(String[] args) {
        ...
        while(t-- > 0)
        {
            ...
            int min[] = new int[2];
            int max[] = new int[2];
            //There will be maxLeft; if it cannot go left;
            int val = (int)(Math.pow(10,5));
            int maxLeft = 0-val; //it can go all the way;
            int minUp = val;
            int minRight = val;
            int maxDown = 0-val;
            for(int i=0;i<n;i++)
            {
                ...
                {
                    //It cannot go left;
                    maxLeft = Math.max(maxLeft,a[i][0]); //It cannot go left beyond this;
                }
                if(a[i][3]==0)
                {
                    minUp = Math.min(minUp,a[i][1]);
                }
                if(a[i][4]==0)
                {
                    minRight = Math.min(minRight,a[i][0]);
                }
                if(a[i][5]==0)
                {
                    maxDown = Math.max(maxDown,a[i][1]);
                }  
            }
            ...
            {
                //System.out.print(1);
                System.out.println(1+" "+ maxLeft +" "+maxDown);
            }
        }
