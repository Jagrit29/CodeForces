...
public class Main {
	...
	public static void main(String[] args) {
        ...
        //Now I want element with degree equal to 1;
        List<String> edges = new ArrayList<>();
        
        //Okay so I will add the edges nodes with degree one to queue;
        //And will dec degree accordingly and see if anyones degree become 1 so will add it to queue;
        //I will add the index in the queue;
        Queue<Integer> q=new LinkedList<>();
        for(int i=0;i<n;i++)
        {
            if(a[i][0]==1)
            {
                q.add(i); //this index I need;
            }
        }
        
        while(q.size()>0)
        {
            int u = q.poll(); //this is u
            if(a[u][0]<=0) continue;
            //Now this element has 1 edge right;
            //This is the edge;
            String edge = u + " " + a[u][1];
            edges.add(edge);
            
            //Now I need to decrease the degree of a[ind][1] and also xor this value to its a[ind][1] so that this edge gets cancelled;
            
            int v = a[u][1];
            
            //Decreased its indegree;
            a[v][0]--;
            a[v][1] = a[v][1] ^ u; //removing this edge;
            if(a[v][0]==1)
            {
                q.add(v);
            }
