...
public class Main {
	...
	public static void main(final String[] args) {
		...
		for(int i=1;i<=n;i++)
		{
			//max j value can be either n-i; if we are in starting;
			int option1=n-i;
			int option2=i-1; //if i=n then j will be 1, so its n-1;
			int div=Math.max(option1,option2);
			min=Math.min(min,a[i-1]/div);
		}
