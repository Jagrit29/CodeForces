...
public class Main {
	...
	public static void main(String[] args) {
        ...
        long sum = 0;
        for(int i : a) sum+=i;
        int each = (int)(sum/n); //this is what each element should be;
        int add = 0;
        if(sum%n!=0)
        {
            add = (int)(sum%n); //Now these elements should be each+1;
        }
        int less = n - add; //elems equal to each;
        int gre = add; //elements equal to each + 1;
        int count = 0;
        sort(a);
        //should be each
        for(int i=0;i<less;i++)
        {
            count = count + Math.abs(each-a[i]);
        }
        //should be each+1;
        for(int i=less;i<n;i++)
        {
            
            count = count + Math.abs((each+1)-a[i]);
        }
