...
public class Main {
	...
	public static void main(String[] args) {
        ...
        for(int i=0;i<n;i++)
        {
            diff[i] = a[i]-b[i]; //this is howmuch I need extra;
        }
        ...
        while(j>=0)
        {
            if(diff[j]>0)
            {
                //How many it can satisfy;
                long add = 0 - (diff[j]-1);
                //Now I need to find this index in diff;
                int ind = bs(diff,add); //index of element equal or this or just greator;
                count = count + (j-ind);
