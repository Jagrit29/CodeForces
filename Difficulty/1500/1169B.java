...
public class Main {
	...
	public static void main(String[] args) {
        ...
        //x will be either one of x and y, like doesnt matter which I choose;
        int x = a[0][0];
        int y = a[0][1];
        int pairsWithoutX=0; //mean they must have y;
        int pairsWithoutY=0;
        for(int i=1;i<m;i++)
        {
            if(a[i][0]!=x && a[i][1]!=x)
            ...
            if(a[i][0]!=y && a[i][1]!=y)
            {
                hm2.put(a[i][1],hm2.getOrDefault(a[i][1],0)+1);
                hm2.put(a[i][0],hm2.getOrDefault(a[i][0],0)+1);
                pairsWithoutY++;
                //System.out.println("hello");
            }
        ...
        {
            max1 = Math.max(max1,ele.getValue());
        }
        for(Map.Entry<Integer,Integer> ele : hm2.entrySet())
        {
            max2 = Math.max(max2,ele.getValue());
        }
        //System.out.println(max);
        if(max1==pairsWithoutX || max2==pairsWithoutY)
        {
