...
public class Main {
	...
	public static void main(String[] args) {
        ...
        while(t-- > 0)
        {
            ...
            int sum = 0; //Sum will remain same in each 
            for(int i: a) sum+=i;
            
            
            //I can either make in 0 to n-1 steps;
            for(int i=0;i<n;i++)
            {
                if(canDo(a,i,sum))
                {
    ...
    public static boolean canDo(int a[], int steps, int sum)
    {
        int left = a.length - steps; //These many elements are left;
        ...
        {
            curSum = curSum + a[i];
            if(curSum==make) curSum=0;
            if(curSum>make) return false;
        }
        return true;
    }
