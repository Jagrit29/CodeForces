...
public class Main {
	...
	public static void main(String[] args) {
        ...
        int split = n/2; //It is always optimal to split at this  //1234  (4/2) = 2;  //123  3/2 = 1;  2;
        //System.out.println(String.valueOf(ch).substring(0,split)+" "+String.valueOf(ch).substring(split));
        //Now optimally this split should give the answer but there can be zeros;
        if(ch[split]!='0')
        {
              //I need to try for split and split -1;
            //long sum1=Long.MAX_VALUE;
            List<Integer> list = solve(ch,split);
            if(split<n-1 && ch[split+1]!='0') 
            {
                List<Integer> list2=solve(ch,split+1);
                list = comp(list,list2);
            }
            //sum2 = solve(ch,split);
            printList(list);
        ...
        else
        {
            ...
            while(left>=1 && ch[left]=='0')
            {
                left--;
            }
            while(right<n && ch[right]=='0') right++;
            //System.out.println(String.valueOf(ch).substring(0,left)+" "+String.valueOf(ch).substring(left));
            //System.out.println(String.valueOf(ch).substring(0,right)+" "+String.valueOf(ch).substring(right));
            //List<Integer> list = new ArrayList<>();
            if(left>=1 && right<n)
            {
                List<Integer> list = new ArrayList<>();
                list = solve(ch,left);
                List<Integer> list2=solve(ch,right);
                list = comp(list,list2);
                printList(list);
            }
            else if(right<n)
            ...
            {
                List<Integer> list2=solve(ch,left);
                printList(list2);
            }
