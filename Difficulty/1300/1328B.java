...
public class Main {
	...
	public static void main(String[] args) {
        ...
        while(t-- > 0)
        {
            ...
            long k = sc.nextInt();
            //aaabb;  here n = 5;
            //for each n there will be n-1 pos for b;
            long a[] =  new long[n-1];
            for(int i=0;i<n-1;i++)
            {
                a[i]=i+1; //for each value i, there possible values of second b;
            }
            long cm[]=new long[n-1];
            cm[0]=a[0];
            for(int i=1;i<n-1;i++)
            {
                cm[i]=cm[i-1]+a[i];
            }
            int index = binarySearch(cm,k); //where will k come;
            char ch[]=new char[n];
            Arrays.fill(ch,'a');
