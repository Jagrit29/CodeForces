...
public class Main {
	...
	public static void main(String[] args) {
        ...
        //Now Each string can contribue digit, alpha, or else;
        int a[][] = new int[n][3];  //[0]-digit, [1] = alpha, [2] = else;
        for(int i=0;i<n;i++)
        {
            char ch[] = s[i].toCharArray();
            a[i][0]=(int)(Math.pow(10,5));
            a[i][1]=(int)(Math.pow(10,5));
            a[i][2]=(int)(Math.pow(10,5));
            for(int j=0;j<m;j++)
            {
                if(ch[j]>='0' && ch[j]<='9')
                {
                    a[i][0]=Math.min(a[i][0],j-0);
                }
                else if(ch[j]>='a' && ch[j]<='z')
                {
                    a[i][1]=Math.min(a[i][1],j-0);
                }
                else
                {
                    a[i][2]=Math.min(a[i][2],j-0);
                }
            }
            for(int j=m-1;j>=0;j--)
            {
                if(ch[j]>='0' && ch[j]<='9')
                {
                    a[i][0]=Math.min(a[i][0],m-j);
                }
                else if(ch[j]>='a' && ch[j]<='z')
                {
                    a[i][1]=Math.min(a[i][1],m-j);
                }
                else
                {
                    a[i][2]=Math.min(a[i][2],m-j);
                }
            }
        }
        long ans = (int)(Math.pow(10,5));
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<n;j++)
            {
                for(int k=0;k<n;k++)
                {
                    ...
                    long count = (long)(a[i][0] + a[j][1] + a[k][2]);
