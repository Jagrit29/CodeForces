...
public class Main {
	...
	public static void main(String[] args) {
        ...
        //minimum value of med can be;
        long low = list.get(0);
        
        long high = low + k;
        
        //checking for all the values from low to low + k; //these can be the pos vaues of med;
        long ans = low; //this can be the ans of the medium;
        while(low<=high)
        {
    ...
    public static boolean check(long med, List<Long> list, int k)
    {
        long add=0;
        for(int i=0;i<list.size();i++)
        {
            if(med-list.get(i)>0)
            {
                // I need to add med-list.get(i) to list.get(i);
                add = add + (med-list.get(i));
            }
            if(add > k)
            {
