        ...
        while(t-- > 0)
        {
            ...
            for(int i=0;i<n;i++)
            {
                ...
                if(nm>=max1 && nm<=min2)
                {
                    ...
                    //I can give this to 1, but I need to check if there is any element left which is less than this;
                    boolean yes = true;
                    for(int j=0;j<nm;j++)
                    {
                        if(count[j]>0) yes=false;
                    }
                    if(yes)
                    {
                        placed=true;
                        list.add(1);
                        count[nm]--;
                        max1 =  Math.max(max1,nm);
                        continue;
                    }
                }
                //I will come here, if I cannot give it two 1;
                if(nm>=max2 && nm>=max1)
                {
                    list.add(2);
                    count[nm]--;
                    min2=Math.min(min2,nm);
                    max2=Math.max(nm,max2);
                    continue;
                }
                pos=false;
                break;
