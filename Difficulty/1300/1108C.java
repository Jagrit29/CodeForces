...
public class Main {
	...
	public static void main(String[] args) {
        ...
        for(int i=0;i<n;i++)
        {
            int r = i%3;
            c[r].add(i);
            //c[i%3].add(i); //like each multiple of there have 3 different colors;
        }
        //System.out.println(Arrays.toString(c[0].toArray()));
       // System.out.println(Arrays.toString(c[1].toArray()));
        //System.out.println(Arrays.toString(c[2].toArray()));
        //Or I can try all the combinations and select the minimum one;
        int make[]=new int[3];
        int min=Integer.MAX_VALUE;
        for(int i=0;i<3;i++)
        {
            int cost=0;
            //cost to make ith red;
            for(int li: c[i])
            {
                if(ch[li]!='R') cost++;
            }
            for(int j=0;j<3;j++)
            {
                ...
                for(int k=0;k<3;k++)
                {
                    ...
                    //now here I need to check;
                    if(cost+cost2+cost3<min)
                    {
                        make[0]=i;
                        make[1]=j;
                        make[2]=k;
                        min=cost+cost2+cost3;
                    }
