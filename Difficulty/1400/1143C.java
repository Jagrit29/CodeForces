...
public class Main {
	public static void main(String[] args) {
        ...
        for(int i=1;i<=n;i++)
        {
            //finding childs of each node from 1 to n;
            ...
            else
            {
                ...
                newList.add(i);
                hm.put(parent,newList);
            }
        }
        List<Integer> ans=new ArrayList<>();
        for(int i=1;i<=n;i++)
        {
            // i==1 doestnt respect its parent;
            if(input[i][1]==1)
            {
                //now it doesnt repsect;
                if(hm.containsKey(i)==false)
                {
                    //System.out.println("hello");
                    //which means its leaf;
                    ans.add(i);
                }
                else
                {
                    //System.out.println(Arrays.toString(hm.get(i).toArray()));
                    boolean resp=true;
                    for(int li: hm.get(i))
                    {
                        if(input[li][1]==0)
